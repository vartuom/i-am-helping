    ...
    return (
        <div className={cls.dropdown_container}>
            <div onClick={toggle} className={cls.dropdown_input}>
                <div className={cls.dropdown_selected_value}>{getSelectValue()}
                </div>
                <div className={cls.dropdown_tools}>
                </div>
                {isOpen && <div className={cls.dropdown_menu}>
                    {options.map((option) => (
                        <div
                            className={`${cls.dropdown_item} ${selectedValue?.value === option.value ? 'selected' : ''}`}
                            onClick={() => { handleItemSelectClick(option) }}
                            key={option.value}
                        >
                            {option.label}
                        </div>
